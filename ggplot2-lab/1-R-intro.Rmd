...
```{r}
babynames_data %>% filter(name=="Alaysia") 
```
...
```{r}
babynames_data %>% filter(name=="Alaysia" & year==2000)
```
...
```{r}
babynames_data |> filter(sex=="F") |> group_by(name) |> summarize(total = sum(n)) |> arrange(desc(total))
